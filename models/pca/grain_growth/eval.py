...
for ncomp in ncomp_list:
    ...
    #save arrays
    np.save("graingrowth_"+str(ncomp)+"_pca", test_recon)
    np.save("graingrowth_"+str(ncomp)+"_pca_z", test_z)
    np.save("graingrowth_"+str(ncomp)+"_pca_eigenvec", pca_model.components_)
    np.save("graingrowth_"+str(ncomp)+"_pca_eigenval", pca_model.explained_variance_)
    ...
    for i in range(test_data.shape[0]):
        if (counter < 100):
            ...
            data_recon = test_recon[i].reshape((256, 256))
            #plotting 
