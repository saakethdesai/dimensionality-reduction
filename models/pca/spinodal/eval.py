...
#ncomp loop
ncomp_list = [10, 50, 100, 500, 1000, 2000, 5000, 7500]
for ncomp in ncomp_list:
    ...
    #save arrays
    np.save("spd_"+str(ncomp)+"_pca", test_recon)
    np.save("spd_"+str(ncomp)+"_pca_z", test_z)
    np.save("spd_"+str(ncomp)+"_pca_eigenvec", pca_model.components_)
    np.save("spd_"+str(ncomp)+"_pca_eigenval", pca_model.explained_variance_)
