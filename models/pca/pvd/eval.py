...
for ncomp in ncomp_list:
    ...
    #save arrays
    np.save("pvd_"+str(ncomp)+"_pca", test_recon)
    np.save("pvd_"+str(ncomp)+"_pca_z", test_z)
    np.save("pvd_"+str(ncomp)+"_pca_eigenvec", pca_model.components_)
    np.save("pvd_"+str(ncomp)+"_pca_eigenval", pca_model.explained_variance_)
