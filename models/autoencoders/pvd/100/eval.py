...
for i, xx in enumerate([test_data, train_data]):
    for batch_idx, sample in enumerate(xx):
        ...
        im1 = ax1.imshow(data_gt, origin='lower')
        divider = make_axes_locatable(ax1)
        cax = divider.append_axes('left', size='5%', pad=0.05)
        fig.colorbar(im1, cax=cax, orientation='vertical');
        im2 = ax2.imshow(data_recon, origin='lower')
        ...
        if (i == 0):
            ...
            string = "train"
        plt.savefig(string+str(batch_idx)+".png")
        plt.close()
