...
class AE(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.econv1 = nn.Conv2d(1, 4, kernel_size=3, stride=1, padding=1) #8x512x512
        self.epool1 = nn.MaxPool2d(2) #8x256x256
        self.econv2 = nn.Conv2d(4, 8, kernel_size=3, stride=1, padding=1) #16x256x256
        self.epool2 = nn.MaxPool2d(2) #16x128x128
        self.econv3 = nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1) #32x128x128
        self.epool3 = nn.MaxPool2d(2) #32x64x64
        self.econv4 = nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1) #32x64x64
        ...
        self.elinear0 = nn.Linear(32*16*16, latent_dim)
        self.dlinear0 = nn.Linear(latent_dim, 32*16*16)
        
        self.dconv1 = nn.ConvTranspose2d(32, 32, kernel_size=2, stride=2) #32x32x32
        self.dconv2 = nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2) #32x64x64
        self.dconv3 = nn.ConvTranspose2d(16, 8, kernel_size=2, stride=2) #16x128x128
        self.dconv4 = nn.ConvTranspose2d(8, 4, kernel_size=2, stride=2) #8x256x256
        self.dconv5 = nn.ConvTranspose2d(4, 2, kernel_size=2, stride=2) #4x512x512
        self.dconv6 = nn.ConvTranspose2d(2, 1, kernel_size=3, stride=1, padding=1) #1x512x512
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(nbatches_train):
        ...
        sample = sample.reshape((batch_size, 1, 512, 512))
