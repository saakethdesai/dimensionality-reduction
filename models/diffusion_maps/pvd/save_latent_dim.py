...
def decoder(args,opt_m):
    ...
    np.save(VAR_DIR+"/test_dmaps_projection", beta)
    del X_train, phi

    # DMAPS Reconstruction
    phi_test = np.load(VAR_DIR+"/latent_unshuffuled.npy")[:,:opt_m]
    print(phi_test.shape, beta.shape)
...
if __name__ == '__main__':
    ...
    encoder(orig, args)
    y_reconst = decoder(args, latent)

    np.save("/qscratch/ashriva/Experiments/Code/dim_reduction/results/latest/pvd_unshuffled_dmaps_"+str(latent), y_reconst)