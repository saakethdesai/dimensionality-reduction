...
basis_path2 = "/qscratch/saadesa/dimensionality_reduction/results_for_unshuffled_data/grain_growth/"

hdffile = DATA_DIR+"graingrowth_256.h5"
hf = h5py.File(hdffile, 'r')
no_samples = hf['test'].shape[0]

time_steps = 20
data_type = "graingrowth"
...
for n in range(len(start)-1):
    ...
    # --------------------------------------------------------------------------------
    # DMAPS
    z_dmaps = np.load(basis_path+"/graingrowth_dmaps_z.npy")
    ...
    # PCA
    z_pca = np.load(basis_path+"graingrowth_"+str(PCA_NDim)+"_pca_z.npy")
    ...
    # AE
    z_ae = np.loadtxt(basis_path2+"AE/graingrowth_"+str(AE_Ndim)+"_ae_latent_space.txt")
    ...
    for i in range(3):
        ...
        if(i==2):
            ...
            axs_twin[i].set_yticks([0, 0.5, 1, 1.5])
        else:
