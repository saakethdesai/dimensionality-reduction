...
def index_closest_to_mean(array):
    mean = np.mean(array)
...
basis_path2 = "/qscratch/saadesa/dimensionality_reduction/results_for_unshuffled_data/pvd/"

hdffile = DATA_DIR+"pvd_unshuffled_data.h5"
hf = h5py.File(hdffile, 'r')
no_samples = hf['data'].shape[0]
...
# for n in [91]:
    ...
    # --------------------------------------------------------------------------------
    # DMAPS
    z_dmaps = np.load(basis_path+"/pvd_dmaps_z.npy")
    ...
    # PCA
    z_pca = np.load(basis_path2+"PCA/pvd_unshuffled"+str(PCA_NDim)+"_pca_z.npy")
    print(n, start[n], start[n+1], z_pca.shape, no_samples)
    corners = cusps(axs[0], z_pca[start[n]:start[n+1], :PCA_NDim], "dmaps")
    angle = slope_direct(axs_twin[0], z_pca[start[n]:start[n+1], :PCA_NDim], "dmaps")
    ...
    # AE
    z_ae = np.loadtxt(basis_path2+"AE/pvd_100_ae_latent_space.txt")
    print(n, start[n], start[n+1], z_ae.shape, no_samples)
    corners = cusps(axs[1], z_ae[start[n]:start[n+1], :AE_Ndim], "dmaps")
    angle = slope_direct(axs_twin[1], z_ae[start[n]:start[n+1], :AE_Ndim], "dmaps")
    ...
    for i in range(3):
        ...
        axs[i].grid('on')
        axs[i].grid(color='k', linestyle='-.', linewidth=2)

        if(i==2):
            axs_twin[i].set_ylabel(r'$\alpha(t)$', fontsize=45, color='red')
            axs_twin[i].set_yticks([0, 0.5, 1, 1.5, 2, 2.5, 3])
        else:
...
print(index_closest_to_mean(corners_array_pca),
      index_closest_to_mean(corners_array_ae),
      index_closest_to_mean(corners_array_dmaps),
      index_closest_to_mean(angle_array_pca),
      index_closest_to_mean(angle_array_ae),
      index_closest_to_mean(angle_array_dmaps))
